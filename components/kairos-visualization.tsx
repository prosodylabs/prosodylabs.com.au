...
export default function KairosVisualization({ network, onSampleLoaded }: KairosProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Randomly choose network and sample if not specified
      const net = network || (Math.random() < 0.5 ? "spiking" : "transformer")
      const sampleIdx = Math.floor(Math.random() * 6)
      const [metaLines, sampleLines] = await Promise.all([
        streamJsonl(`/kairos/${net}_meta.jsonl`),
        streamJsonl(`/kairos/${net}_sample_${sampleIdx}.jsonl`),
      ])
      if (disposed) return
